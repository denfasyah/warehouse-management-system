@extends('layouts.app')
@section('title', 'Penataan Barang CBS - WMS Petugas')
...
<div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h2 class="text-2xl font-headline-lg font-bold text-gray-800">Penataan Barang (CBS)</h2>
        <p class="text-sm text-gray-500 mt-1">Panduan penataan tata letak barang berdasarkan skor frekuensi dan kelas penyimpanannya.</p>
    </div>
</div>
...
<div class="mb-6 bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex gap-2 overflow-x-auto">
    <a href="{{ route('petugas.cbs.arrangement') }}" class="px-4 py-2 rounded-lg text-sm font-semibold whitespace-nowrap {{ !request('class') ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">Semua Kelas</a>
    <a href="{{ route('petugas.cbs.arrangement', ['class' => 'fast']) }}" class="px-4 py-2 rounded-lg text-sm font-semibold whitespace-nowrap {{ request('class') == 'fast' ? 'bg-red-500 text-white' : 'bg-red-50 text-red-700 hover:bg-red-100' }}">Fast Moving</a>
    <a href="{{ route('petugas.cbs.arrangement', ['class' => 'medium']) }}" class="px-4 py-2 rounded-lg text-sm font-semibold whitespace-nowrap {{ request('class') == 'medium' ? 'bg-yellow-500 text-white' : 'bg-yellow-50 text-yellow-700 hover:bg-yellow-100' }}">Medium Moving</a>
    <a href="{{ route('petugas.cbs.arrangement', ['class' => 'slow']) }}" class="px-4 py-2 rounded-lg text-sm font-semibold whitespace-nowrap {{ request('class') == 'slow' ? 'bg-green-500 text-white' : 'bg-green-50 text-green-700 hover:bg-green-100' }}">Slow Moving</a>
    <a href="{{ route('petugas.cbs.arrangement', ['class' => 'unclassified']) }}" class="px-4 py-2 rounded-lg text-sm font-semibold whitespace-nowrap {{ request('class') == 'unclassified' ? 'bg-gray-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">Unclassified</a>
</div>
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                @php
                    ...
                    $suggestions = $isMismatch ? \App\Services\CBSService::suggestLocations($item) : collect();
                @endphp
                <tr class="hover:bg-gray-50/50 transition-colors {{ $isMismatch ? 'bg-orange-50/30' : '' }}">
                    ...
                    <td class="px-6 py-4 text-center">
                        @if($isMismatch)
                            <span class="inline-flex flex-col items-center gap-1 px-3 py-2 bg-orange-50 text-orange-700 border border-orange-200 rounded-lg text-xs font-semibold mb-1">
                                <span class="material-symbols-outlined text-[20px]">warning</span>
                                Perlu Relokasi
                            </span>
                            @if($suggestions->isNotEmpty())
                                <p class="text-[10px] text-gray-500 mt-1">Rekomendasi: <span class="font-bold text-primary">{{ $suggestions->first()->code }}</span></p>
                            @else
                                <p class="text-[10px] text-red-500 mt-1">Rak penuh!</p>
                            @endif
                        @else
