@extends('layouts.app')
@section('title', 'Input Barang Masuk - WMS Petugas')
...
<div class="mb-6 flex items-center gap-3">
    ...
    <div>
        <h2 class="text-2xl font-headline-lg font-bold text-gray-800">Catat Barang Masuk</h2>
        <p class="text-sm text-gray-500 mt-0.5">Pencarian barang manual untuk stok masuk (Inbound).</p>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <h3 class="font-bold text-lg text-gray-800 mb-4">Pilih Barang</h3>
        
        <div class="relative mb-4">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <span class="material-symbols-outlined text-gray-400">search</span>
            </div>
            <input type="text" id="searchInput" class="w-full pl-10 rounded-lg border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 text-sm py-2.5" placeholder="Ketik Nama Barang, SKU, atau Barcode..." autocomplete="off">
        </div>

        <div id="searchResults" class="hidden flex-col gap-2 max-h-[400px] overflow-y-auto">
            <!-- Hasil pencarian via JS akan di-inject di sini -->
        </div>
        ...
        <div id="searchIdle" class="text-center py-8">
            ...
            <p class="text-gray-500 text-sm">Mulai ketikkan kata kunci untuk mencari barang master.</p>
        </div>
    </div>

    <!-- Kolom Kanan: Form Input -->
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('petugas.incoming.store') }}" method="POST">
            @csrf
            
            <input type="hidden" name="item_id" id="selectedItemId" value="{{ old('item_id') }}">

            <div class="mb-6 pb-6 border-b border-gray-100">
                <h3 class="font-bold text-lg text-gray-800 mb-4">Detail Barang Masuk</h3>
                
                <div id="selectedItemDisplay" class="{{ old('item_id') ? '' : 'hidden' }} bg-primary/5 border border-primary/20 rounded-xl p-4 flex gap-4 items-start">
            ...
            <div class="space-y-5">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Jumlah Masuk (Kuantitas) <span class="text-red-500">*</span></label>
                    <div class="flex items-center gap-3">
                        <input type="number" name="quantity" min="1" value="{{ old('quantity') }}" placeholder="Contoh: 50" class="w-full md:w-1/2 rounded-lg border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 text-sm py-2.5">
                ...
                <div>
                    ...
                    <textarea name="note" rows="3" class="w-full rounded-lg border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 text-sm" placeholder="Contoh: Barang diterima dari supplier PT XYZ. Kondisi kardus sedikit penyok.">{{ old('note') }}</textarea>
                    @error('note')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-8">
                <button type="submit" id="btnSubmit" class="w-full bg-primary text-white font-bold py-3 px-4 rounded-xl hover:bg-primary/90 transition-colors shadow-md disabled:opacity-50 disabled:cursor-not-allowed" {{ old('item_id') ? '' : 'disabled' }}>
                    Simpan Barang Masuk
                </button>
            </div>
        </form>
                ...
                .then(data => {
                    ...
                    if (data.length === 0) {
                        ...
                        data.forEach(item => {
                            ...
                            div.className = 'p-3 border border-gray-100 rounded-lg hover:border-primary hover:bg-primary/5 cursor-pointer transition-colors flex justify-between items-center';
                            div.innerHTML = `
                                <div>
                                    <p class="text-sm font-bold text-gray-800">${item.name}</p>
                                    <p class="text-xs text-gray-500 font-mono mt-0.5">${item.sku} &bull; ${categoryName}</p>
                                </div>
